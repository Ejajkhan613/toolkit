...
const WordTramsform = () => {
    const [inoutTypeText, setinoutTypeText] = useState("");
    ...
    const handleInputChange = (event) => {
        setinoutTypeText(event.target.value);
    };
    ...
    const handleprice = async () => {
        ...
        let inc_cny = cny_pricing * 40 / 100;
    ...
    const transformText = () => {
        ...
        setOutputText(transformedWords.join(" "));
    ...
    return (
        <Box display={"flex"} w={"90%"} margin={"auto"} gap={20} >
            <Box flex={5} m={"auto"}>
                <img src={tree} id="background-video" alt="backimg" />
                <div>
                    <Box className="white_grid">
                        <h2>First word Transformer </h2>
            ...
            {/* ======================== pricing box ======================== */}
            <Box display={"grid"} flex={2} backgroundColor={"green"} padding={"20px"} borderRadius={10} color={"yellow"}>
